...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		arr = new int[N+1];//답으로 출력할 정점 깊이 저장할 배열
		for(int i=0; i<N+1; i++) arr[i] = -1;//초기값 -1로!(방문되지 않은 노드의 깊이 == -1)
		...
		while(!q.isEmpty()) {
			...
			for(int vertex : adjList[curr]) {//탐색 정점의 인접 정점에 대해서
				if(!visited[vertex]) {//아직 방문하지 않은 정점이면
					...
					arr[vertex] = arr[curr]+1;//나의 정점 깊이 = 나를 불러낸 정점 깊이+1
				}
			}
		}
	}
