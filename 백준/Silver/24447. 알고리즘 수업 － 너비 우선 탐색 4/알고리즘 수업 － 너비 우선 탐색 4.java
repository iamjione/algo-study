...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		order = new long[N+1];//방문순서 저장할 배열
		level = new long[N+1];//정점 깊이 저장할 배열
		//방문되지 않는 정점은 방문순서 0 이기 때문에 정점 깊이 -1 곱해도 0이라서 초기값 따로 설정 안해줘도 됨
		...
		long sum = 0;
		for(int i=1; i<N+1; i++) sum += order[i]*level[i];
		...
		while(!q.isEmpty()) {
			...
			for(int vertex : adjList[curr]) {//탐색 정점의 인접 정점에 대해서
				if(!visited[vertex]) {//아직 방문하지 않은 정점이면
					...
					level[vertex] = level[curr]+1;//나의 정점 깊이 = 나를 불러낸 정점 깊이+1
				}
			}
		}
	}
